...
def cal_feature(df):
    ...
    for w in window:
        df["beta_{}".format(w)] = (df["close"].shift(w) - df["close"]) / (
            w * df["close"]
        )
    ...
    for w in window:
        df["sump_{}".format(w)] = df["pos_ret1"].rolling(w).sum() / (
            df["abs_ret1"].rolling(w).sum() + 1e-12
        )
    ...
    df.drop(
        columns=["ret1", "abs_ret1", "pos_ret1", "vchg1", "abs_vchg1", "pos_vchg1"],
        inplace=True,
    )
...
if __name__ == "__main__":
    ...
    main(
        os.path.join(ROOT, "datasets", "mcad", "raw"),
        os.path.join(ROOT, "datasets", "mcad", "features"),
    )
